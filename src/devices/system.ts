...
const errors: string[] = ['underflow', 'overflow', 'division by zero'];
...
export const uxn_halt = async (
  ...
  if (handler) {
    ...
    system_inspect(u);
    outError(
      `${instr & 0x40 ? 'Return-stack' : 'Working-stack'} ${
        errors[err - 1]
      }, by ${instr.toString(16)} at 0x${addr.toString(16).padStart(4, '0')}.\n`
    );
  }
  return 0;
};
